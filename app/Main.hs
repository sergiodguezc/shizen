...
main = do
    ...
    b1 <- ACO.antColony as1 na1 ss1 True bp1 evaporationRate ni1
    putStrLn "Global minimum: x = 3, f(3) = -3"
    print $ CPU.run b1
    putStrLn "Benchmark 2"
    b2 <- ACO.antColony as2 na2 ss2 True bp2 evaporationRate ni2
    putStrLn "Global minimum: (x, y) = (0, 0), f(0,0) = 0"
    print $ CPU.run b2
    putStrLn "Benchmark 3"
    b3 <- ACO.antColony as3 na3 ss3 True bp3 evaporationRate ni3
